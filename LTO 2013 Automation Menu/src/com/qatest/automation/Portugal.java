...
public class Portugal {
  ...
  public Portugal()
  {
      ...
      data.setLogin("autoLTOPT@gmail.com");
      data.setPassword("abc123");
      data.setDistID("PT3400609");
      data.setBuyerID("none");
  }
  public Portugal(BuyerDataForm d)
  {
      data = d;
  }
  ...
  @Test
  public String[] testPortugal(boolean place, boolean screenshot, String location) throws Exception {
  ...
  public String[] Myself(boolean place, boolean screenshot, String location)
  {
	  try{
		//global landing page
		driver.get(baseUrl);
		driver.findElement(By.linkText("Portugal")).click();
		//United States landing page - Order Now button
		LandingPage land = new LandingPage(driver);
		results[0] = land.landing(data.getLogin(), data.getPassword());
		if (results[0] != null)
		{
			results[0] = "Portugal: Failed: Myself\n" + results[0];
			if (screenshot)
				myScreenShot.takeScreenShot(location, "Portugal");
			return results;
		}
	    ...
	    results[0] = "Portugal: Passed";
        results[3] = "ExecutiveID: " +data.getDistID() + " BuyerID: " + data.getBuyerID();
        String[] ordernum = results[1].split(" ");
        LTO2013Menu.getLog().appendLog(data.getDistID(), data.getBuyerID(), ordernum[3], "Portugal");
	    return results;
	  }
	  
	  catch (Exception e)
	  {
		  results[0] = "Portugal: Script Error: Buy for myself\n"+ "URL: " + driver.getCurrentUrl() + "\n" + "Script Error: " + e;
		  if (screenshot)
				myScreenShot.takeScreenShot(location, "Portugal");
		  return results;
	  }
