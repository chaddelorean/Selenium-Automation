...
public class CzechRepublic {
  ...
  @Test
  public String[] testCzechRepublic(boolean place, boolean screenshot, String location) throws Exception {
  ...
  public String[] Myself(boolean place, boolean screenshot, String location)
  {
	  try{
		//global landing page
		driver.get(baseUrl);
		driver.findElement(By.linkText("Česká Republika (Czech Republic)")).click();
		//United States landing page - Order Now button
		LandingPage land = new LandingPage(driver);
		results[0] = land.landing(data.getLogin(), data.getPassword());
		if (results[0] != null)
		{
			results[0] = "Czech Republic: Failed: Myself\n" + results[0];
			if (screenshot)
        ...
        LTO2013Menu.getLog().appendLog(data.getDistID(), data.getBuyerID(), ordernum[3], "Czech Republic");
	    return results;
	  }
	  
	  catch (Exception e)
	  {
		  results[0] = "Czech Republic: Script Error: Buy for myself\n"+ "URL: " + driver.getCurrentUrl() + "\n" + "Script Error: " + e;
		  if (screenshot)
				myScreenShot.takeScreenShot(location, "CzechRepublic");
		  return results;
	  }
