...
public class Norway {
  ...
  @Test
  public String[] testNorway(boolean place, boolean screenshot, String location) throws Exception {
  ...
  public String[] Myself(boolean place, boolean screenshot, String location)
  {
	  try{
		//global landing page
		driver.get(baseUrl);
		driver.findElement(By.linkText("Norge (Norway)")).click();
		//United States landing page - Order Now button
		LandingPage land = new LandingPage(driver);
		results[0] = land.landing(data.getLogin(), data.getPassword());
		if (results[0] != null)
		{
			results[0] = "Norway: Failed: Myself\n" + results[0];
			if (screenshot)
				myScreenShot.takeScreenShot(location, "Norway");
			return results;
		}
	    ...
	    results[0] = "Norway: Passed";
        results[3] = "ExecutiveID: " +data.getDistID() + " BuyerID: " + data.getBuyerID();
        String[] ordernum = results[1].split(" ");
        LTO2013Menu.getLog().appendLog(data.getDistID(), data.getBuyerID(), ordernum[3], "Norway");
	    return results;
	  }
	  
	  catch (Exception e)
	  {
		  results[0] = "Norway: Script Error: Buy for myself\n"+ "URL: " + driver.getCurrentUrl() + "\n" + "Script Error: " + e;
		  if (screenshot)
				myScreenShot.takeScreenShot(location, "Norway");
		  return results;
	  }
